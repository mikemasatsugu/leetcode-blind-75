...
# check to see if they passed in which problem they want to test
if [ -z $1 ]; then
  echo -en '\n'
  echo -e "\033[0;31mMake sure to format the command properly:\033[0m"
  echo -en '\n'
  echo -e "\033[01;36mCorrect Syntax:\033[0m 'npm run testproblem <problem name>' "; 
  echo -en '\n'
...
# run the test file
  ...
  echo -en '\n'
  # if they passed the problem to test in as <problem>.js, then don't add .js to filename
  if [[ $1 == *".js"* ]]; then
    ./node_modules/.bin/jest __tests__/$1
  # if they followed instructions and passed in just the name of the problem:
  else
